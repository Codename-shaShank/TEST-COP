...
def fetch_base_lockfile
  # Ensure we have the base branch locally (but don't fail if fetch fails)
  system('git', 'fetch', 'origin', BASE_REF) rescue nil

  begin
    content = IO.popen(['git', 'show', "origin/#{BASE_REF}:Gemfile.lock"], err: [:child, :out], &:read)
  rescue
    warn "Could not read origin/#{BASE_REF}:Gemfile.lock, attempting local fallback..."
    # Try to read from local ref if remote fails
    begin
      content = `git show #{BASE_REF}:Gemfile.lock 2>/dev/null` rescue ""
    rescue
      content = ""
    end
  end
  ...
  if content.strip.empty?
    warn "Could not read #{BASE_REF}:Gemfile.lock. Skipping gem diff analysis."
    exit 0
  end
  content
end
...
if changes.empty?
  begin
    output = "No gem version changes detected between this PR and #{BASE_REF}."
    File.write(gem_review_path, output)
    puts output
    exit 0  # Success - no changes is not an error
  rescue StandardError => e
    # If we can't write the file, just warn and continue (don't fail the workflow)
    warn "Warning: Could not write gem_review.md: #{e.message}"
    warn "Attempted path: #{gem_review_path}"
    puts "No gem changes detected (file write skipped)"
    exit 0
  end
end
...
begin
  ...
  # If we can't write the file, just warn and continue
  warn "Warning: Could not write gem_review.md: #{e.message}"
  warn "Attempted path: #{gem_review_path}"
  puts markdown  # Still output the markdown to stdout
  exit 0
end
